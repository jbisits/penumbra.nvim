local function load_old_config()
  ...
  if using_old_config then
    vim.schedule(function()
      vim.notify(messages, vim.log.levels.WARN, { title = 'penumbra.nvim' })
    end)
    -- some values need to be inverted before applying them to new config
    if cfg.disable_terminal_colors ~= nil then
      cfg.disable_terminal_colors = not cfg.disable_terminal_colors
    end
    if cfg.hide_ending_tildes ~= nil then
      cfg.hide_ending_tildes = not cfg.hide_ending_tildes
    end
    if cfg.italic_comment ~= nil then
      cfg.italic_comment = cfg.italic_comment and 'italic' or 'none'
    end
    local new_config = {
      style = cfg.style,
